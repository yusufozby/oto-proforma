...
const TEXT_SECONDARY: [number, number, number] = [
  100,
  100,
  100,
];
...
const ORANGE_DARK: [number, number, number] = [
  210,
  115,
  20,
];
...
const CARD_BORDER: [number, number, number] = [
  225,
  225,
  225,
];
...
const TABLE_HEADER_BG: [number, number, number] = [
  248,
  249,
  250,
];

const ROW_ALT_BG: [number, number, number] = [
  252,
  252,
  253,
];
...
async function preloadProductImages(
  ...
  await Promise.all(
    products.map(async (row) => {
      ...
      // Şimdilik ürün kodu olmadığı için
      // product_id üzerinden görsel aranmıyor.
      //
      // API / Product bilgisi geldiğinde burası:
      // product.code -> `${product.code}.jpg`
      //
...
function assertValidTtf(
  buffer: ArrayBuffer,
  label: string
) {
  if (buffer.byteLength < 1000) {
    throw new Error(
      `${label} dosyası çok küçük.`
    );
  }
  ...
  if (!validMagics.includes(magic)) {
    throw new Error(
      `${label} geçerli bir TTF/OTF dosyası değil.`
    );
  }
}
...
async function registerFonts(
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        "Font dosyaları sunucudan alınamadı."
      );
    }
...
function drawCard(
  ...
  if (opts?.header) {
    headerHeight =
      opts.header.height ?? 8;

    const headerColor =
      opts.header.color ?? ORANGE;
...
function drawBuyerAndSeller(
  ...
  const sellerRows = [
    ...
    [
      "MERKEZ",
      safeString(
        session.center_address
      ),
    ],
    [
      "FABRİKA",
      safeString(
        session.fabric_address
      ),
    ],
    [
      "TELEFON",
...
function getProductValue(
  ...
  switch (key) {
    ...
    case "kod":
      // API olmadığı için şimdilik product_id
      return safeString(
        row.product_id
      );

    case "isim":
      // Ürün bilgisi API'den gelmediği için
    ...
    case "totalAdet":
      // Şu an ProductProforma içinde
      // sadece percel olduğu için
      // total adet hesaplanamıyor.
      return safeString(
        row.percel
      );
...
async function drawProductTable(
  ...
  const products: ProductProforma[] =
...
export async function shareProformaPdf(
  ...
  if (
    ...
    throw new Error(
      "Bu cihaz PDF dosyası paylaşmayı desteklemiyor."
    );
  }
